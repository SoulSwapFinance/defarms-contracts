...
contract Manifester is IManifester {
    ...
    constructor(
        address _wnativeAddress, address _nativeOracle, uint _oracleDecimals) {
    ...
    function createManifestation(
        ...
        require(depositToken != address(0), 'depositToken must be SoulSwap LP');
        ...
        // IERC20(rewardToken).safeTransferFrom(msg.sender, soulDAO, sacrifice); // todo: re-enable
        
        // transfers: `totalRewards` to the manifestation contract.
        // IERC20(rewardToken).safeTransferFrom(msg.sender, manifestation, rewards); // todo: re-enable
